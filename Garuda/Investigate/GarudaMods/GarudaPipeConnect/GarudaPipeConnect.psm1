function View-PipeConnectSummary {
...
function View-PipeConnectInteractiveTable {
    [CmdletBinding(PositionalBinding = $false)]
    param (
        [Parameter(Mandatory = $false, ValueFromPipeline)]
        [psobject] $Event
    ...
    Process {
        ...
        $PipeEvents | Select-Object UTCtime,
            ...
            @{Name="Event"; Expression={ "Pipe Connected (18)" }},
            ...
            # Time-related fields
            TimeCreated |
            Out-GridView -Title "Pipe Connection Events"
    }
}

function View-PipeConnectTimeline {
    [CmdletBinding(PositionalBinding = $false)]
    param (
        [Parameter(Mandatory = $false, ValueFromPipeline)]
        [psobject] $Event
...
function View-PipeConnectTimelineList {
    [CmdletBinding(PositionalBinding = $false)]
    param (
        [Parameter(Mandatory = $false, ValueFromPipeline)]
        [psobject] $Event
    ...
    End {
        $PipeEvents | Select-Object `
            ...
            @{Name="Event"; Expression={ "Pipe Connected" }},
            @{Name="EventDetails"; Expression={ $_.PipeName }} |
            Sort-Object UTCtime | Format-List
    }
}

function Investigate-PipeConnectInfo {
    [CmdletBinding(PositionalBinding = $false)]
    param (
    ...
    if ($null -eq $LogFile) {
        $Parameters = @{
            EventId = 18  # Only get ConnectPipe events
    ...
    switch ($View) {
        ...
        "Summary" {
            $Query | View-PipeConnectSummary
            break
        }
        "InteractiveTable" {
            $Query | View-PipeConnectInteractiveTable
            break
        }
        "Timeline" {
            $Query | View-PipeConnectTimeline
            break
        }
        "TimelineList" {
            $Query | View-PipeConnectTimelineList
            break
        }
    }
}

Export-ModuleMember -Function View-PipeConnectSummary, 
                             View-PipeConnectInteractiveTable, View-PipeConnectTimeline, View-PipeConnectTimelineList,
